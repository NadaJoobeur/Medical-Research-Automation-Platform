...
import '../../assets/DLcss/InceptionV2.css';
import axios from 'axios';
import JSZip from 'jszip';
import { saveAs } from 'file-saver';
import '../../assets/DLcss/InceptionV2.css'; // Make sure this path is correct
...
const InceptionV2 = () => {
    ...
    const downloadImages = async () => {
        ...
        for (const url of imageUrls) {
            ...
            imgFolder.file(`original_${url.split('/').pop()}`, blob);
        }

        // Télécharger les images prédites
        for (const result of predictionListe) {
            const response = await fetch(result.url);
            const blob = await response.blob();
            imgFolder.file(`predicted_${result.url.split('/').pop()}`, blob);
        }
    ...
    return (
        <div className="container">
            ...
            <div className="content1">
                <h2>Inception V2 - Model Details</h2>
                <p>Here you can view your model results related to Inception V2.</p>
